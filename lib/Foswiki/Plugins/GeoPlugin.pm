...
our $SHORTDESCRIPTION = 'geo-location services, providing lookup features from IP address, domain name, or physical address';
...
sub initPlugin {
    ...
    Foswiki::Func::registerTagHandler( REMOTE_IP_ADDRESS => \&_REMOTE_IP_ADDRESS );
...
sub _GEOTAG {
    ...
    my $format = $params->{format} || ( '| $query | ' . join( ' | ', map { "\$$_" } @display_fields ) . ' |' ) || '$country_code ($lat,$lng)';
...
#    if ( $query =~ m{^(\d+\.){3}\d+$} or $query =~ m{\.} ) {
	...
	require Geo::IP;
	my $gi = ( $session->{GeoPlugin}->{geo_ip}->{self} ||= Geo::IP->open( '/usr/local/share/GeoIP/GeoLiteCity.dat' ) );	#, 'GEOIP_STANDARD' ) );

	my $cache = $session->{GeoPlugin}->{geo_ip}->{cache}->{ $query }->{result};
...
#	$text .= '<pre>record=' . Dumper($record) . '</pre>';
	...
	my $geo = ( $session->{GeoPlugin}->{geo_geonames} ||= Geo::GeoNames->new() );
